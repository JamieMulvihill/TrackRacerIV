...
public class EngineAudio : MonoBehaviour
{
    ...
    public Controller carController_; // Reference to car we are controlling
    public InputManager inputManager_;

    private void StartSound()
    {
        // get the carcontroller ( this will not be null as we have require component)
        carController_ = GetComponent<Controller>();
    ...
    private void Update()
    {
        ...
        if (m_StartedSound)
        {
            ...
            float pitch = ULerp(lowPitchMin, lowPitchMax, carController_.engineRPM_ / carController_.maxRPM);
            ...
            else
            {
                ...
                float accFade = Mathf.Abs((inputManager_.vertical_ > 0) ? inputManager_.vertical_ : 0);
                float decFade = 1 - accFade;

                // get the high fade value based on the cars revs
                float highFade = Mathf.InverseLerp(0.2f, 0.8f, carController_.engineRPM_ / carController_.maxRPM);
    ...
    private AudioSource SetUpEngineAudioSource(AudioClip clip)
    {
        ...
        source.spatialBlend = .45f;
        source.maxDistance = 125;
